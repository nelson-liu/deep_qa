...
class TestAttentionLayer:
    ...
    def test_no_mask(self):
        ...
        result = model.predict([vector, matrix])
        assert_almost_equal(result, np.array([[0.5, 0.5]]))
    ...
    def test_masked(self):
        ...
        assert_almost_equal(result, np.array([[0.52248482, 0.0, 0.47751518]]))
    ...
    def test_call_works_on_simple_input(self):
        ...
        sentence_tensor = np.asarray([[[1, 1, 1], [-1, 0, 1]]])
        query_tensor = np.asarray([[.1, .8, .5]])
    ...
    def test_call_handles_masking_properly(self):
        ...
        vocab_size = 4
        embedding_dim = 3
        embedding_weights = np.asarray([[0, 0, 0], [1, 1, 1], [-1, 0, 1], [-1, -1, 0]])
        ...
        sentence_tensor = np.asarray([[0, 1, 0, 2]])
        query_tensor = np.asarray([[.1, .8, .5]])
        attention_tensor = model.predict([query_tensor, sentence_tensor])
