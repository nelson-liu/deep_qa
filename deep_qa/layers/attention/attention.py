...
class Attention(MaskedLayer):
    ...
    properly for masked rows in the matrix, though we ignore any masking on the vector.
    ...
    - ``(batch_size, num_rows)``, no mask (masked input rows have value 0 in the output)
